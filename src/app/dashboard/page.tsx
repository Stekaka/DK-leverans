      ...
      <header className="relative bg-gradient-to-r from-slate-900 via-yellow-900 to-slate-900 shadow-tech border-b border-yellow-500/20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-4 sm:py-6 gap-4">
            <div className="flex items-center space-x-3 sm:space-x-6 w-full sm:w-auto">
              <DrönarkompanietLogo variant="full" size="sm" className="text-white sm:size-md" />
              <div className="flex items-center space-x-3 sm:space-x-4 flex-1">
                <div className="w-0.5 h-8 sm:h-12 bg-gradient-to-b from-transparent via-yellow-400/60 to-transparent hidden sm:block"></div>
                <div className="backdrop-blur-sm bg-white/10 rounded-xl px-3 py-2 sm:px-6 sm:py-3 border border-white/20 flex-1 sm:flex-initial">
                  <span className="text-white font-medium text-sm sm:text-base truncate block">
                    {customer?.name}
                  </span>
                  <div className="text-yellow-300 text-xs sm:text-sm font-medium truncate">
                    {customer?.project}
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 w-full sm:w-auto justify-end">
              <button 
                ...
                className="group relative bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-400 hover:to-yellow-500 text-white px-3 py-2 sm:px-6 sm:py-3 rounded-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1 sm:space-x-2 shadow-glow-sm hover:shadow-glow-md border border-yellow-400/50 text-xs sm:text-sm"
              >
                {filteredFiles.length > 1 ? (
                  <>
                    ...
                    <span className="hidden sm:inline">Ladda ner alla som ZIP ({filteredFiles.length})</span>
                    <span className="sm:hidden">ZIP ({filteredFiles.length})</span>
                  </>
                ) : (
                ...
                <button 
                  onClick={logout}
                  className="group relative text-white/70 hover:text-white transition-all duration-300 p-2 sm:p-3 rounded-xl hover:bg-white/10 border border-white/20 hover:border-white/30"
                  title="Logga ut"
                >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
          ...
          {(filteredFiles.length > 10 || formattedTotalSize.includes('GB')) && (
            <div className="relative mb-4 sm:mb-6 backdrop-blur-sm bg-gradient-to-r from-yellow-500/20 to-yellow-500/20 rounded-2xl p-4 sm:p-6 border border-yellow-400/30 shadow-tech overflow-hidden">
              {/* Tech background */}
              ...
              <div className="relative flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-yellow-500 to-yellow-500 rounded-xl flex items-center justify-center shadow-glow-sm flex-shrink-0">
                ...
                {/* Tech accent */}
                <div className="w-1 h-12 sm:h-16 bg-gradient-to-b from-yellow-400 to-yellow-500 rounded-full opacity-60 hidden sm:block"></div>
              </div>
            </div>
          )}
          ...
          {folders.length > 0 && (
            <div className="mb-4 sm:mb-6">
              <div className="backdrop-blur-sm bg-white/10 rounded-2xl shadow-tech p-4 sm:p-6 border border-white/20 relative overflow-hidden">
                {/* Tech background */}
                ...
                <span className="text-xs sm:text-sm font-medium text-gray-200">Mappar:</span>
              </div>
              ...
              {currentFolder && (
                <button
                  onClick={navigateUp}
                  className="mt-2 text-yellow-400 hover:text-yellow-300 text-xs sm:text-sm"
                >
                  ← Tillbaka
                </button>
              )}
        ...
        <div className="mb-6 sm:mb-8">
          <div className="bg-white/10 backdrop-blur-sm rounded-lg shadow-sm p-4 sm:p-6 mb-4 sm:mb-6 border border-white/20">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
              <div className="text-center">
                <div className="text-xl sm:text-3xl font-bold text-yellow-400">{totalFiles}</div>
                <div className="text-xs sm:text-sm text-gray-300">Totala filer</div>
              </div>
              <div className="text-center">
                <div className="text-xl sm:text-3xl font-bold text-green-400">{totalImages}</div>
                <div className="text-xs sm:text-sm text-gray-300">Bilder</div>
              </div>
              <div className="text-center">
                <div className="text-xl sm:text-3xl font-bold text-purple-400">{totalVideos}</div>
                <div className="text-xs sm:text-sm text-gray-300">Videor</div>
              </div>
              <div className="text-center">
                <div className="text-xl sm:text-3xl font-bold text-orange-400">{formattedTotalSize}</div>
                <div className="text-xs sm:text-sm text-gray-300">Total storlek</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 mb-6">
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-xs sm:text-sm text-gray-300 font-medium">Filter:</span>
                  <button
                    onClick={() => setFilter('all')}
                    className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-all ${filter === 'all' ? 'bg-yellow-600 text-white' : 'bg-gray-600 text-gray-200 hover:bg-gray-500'}`}
                  >
                    Alla ({totalFiles})
                  </button>
                  <button
                    onClick={() => setFilter('images')}
                    className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-all ${filter === 'images' ? 'bg-yellow-600 text-white' : 'bg-gray-600 text-gray-200 hover:bg-gray-500'}`}
                  >
                    Bilder ({totalImages})
                  </button>
                  <button
                    onClick={() => setFilter('videos')}
                    className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-all ${filter === 'videos' ? 'bg-yellow-600 text-white' : 'bg-gray-600 text-gray-200 hover:bg-gray-500'}`}
                  >
                    Videor ({totalVideos})
                  </button>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-xs sm:text-sm text-gray-300 font-medium">Betyg:</span>
                  <button
                    onClick={() => setFilter('favorite')}
                    className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm flex items-center space-x-1 transition-all ${filter === 'favorite' ? 'bg-yellow-500 text-white' : 'bg-gray-600 text-gray-200 hover:bg-gray-500'}`}
                  >
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm flex items-center space-x-1 transition-all ${filter === 'good' ? 'bg-green-500 text-white' : 'bg-gray-600 text-gray-200 hover:bg-gray-500'}`}
                  >
                    <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm flex items-center space-x-1 transition-all ${filter === 'poor' ? 'bg-red-500 text-white' : 'bg-gray-600 text-gray-200 hover:bg-gray-500'}`}
                  >
                    <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-all ${filter === 'unrated' ? 'bg-gray-500 text-white' : 'bg-gray-600 text-gray-200 hover:bg-gray-500'}`}
                  >
                    <span className="hidden sm:inline">Ej betygsatt</span>
                    <span className="sm:hidden">Obetygsatt</span> ({totalUnrated})
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
              {selectedItems.length > 0 && (
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-xs sm:text-sm text-gray-300">{selectedItems.length} valda</span>
                  <button
                    onClick={downloadSelected}
                    className="bg-green-600 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm hover:bg-green-700 flex items-center space-x-1 transition-all"
                  >
                    {selectedItems.length > 1 ? (
                      <>
                        ...
                        <span className="hidden sm:inline">Ladda ner ZIP</span>
              ...
              <div className="flex items-center justify-between sm:justify-end space-x-3 sm:space-x-4">
                <button
                  onClick={selectAll}
                  disabled={filteredFiles.length === 0}
                  className="text-yellow-400 hover:text-yellow-300 text-xs sm:text-sm disabled:opacity-50 transition-colors"
                >
                  Välj alla
                </button>
                ...
                <div className="flex items-center space-x-1 sm:space-x-2">
                  <button
                    ...
                    className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-yellow-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500'}`}
                  >
                  ...
                  <button
                    ...
                    className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-yellow-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500'}`}
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
        ...
        {filteredFiles.length === 0 ? (
          <div className="bg-white/10 backdrop-blur-sm rounded-lg shadow-sm p-8 sm:p-12 text-center border border-white/20">
            ...
            <h3 className="text-base sm:text-lg font-medium text-gray-200 mb-2">Inga filer hittades</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {filteredFiles.map((file, index) => (
              <div
                key={file.id}
                className={`bg-white/10 backdrop-blur-sm rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-all cursor-pointer border border-white/20 ${
              ...
              >
                <div className="relative">
                  <div 
                    className="aspect-video bg-gray-700 flex items-center justify-center hover:bg-gray-600 transition-colors"
                    onClick={(e) => {
                      e.stopPropagation()
                      if (file.is_image) {
                        openGallery(index)
                    ...
                    {file.is_image ? (
                      file.thumbnail_url ? (
                        ...
                        <div className="text-center p-4">
                          ...
                          <span className="text-xs text-gray-400 mt-2 block">Tryck för att visa</span>
                        </div>
                      )
                    ) : (
                      <div className="text-center p-4">
                        ...
                        <span className="text-xs text-gray-400 mt-2 block">Tryck för att öppna</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="absolute bottom-2 right-2">
                    <button
                      ...
                      className="bg-black bg-opacity-70 text-white p-2 rounded-lg hover:bg-opacity-90 transition-all shadow-lg"
                      title="Ladda ner"
                    >
                ...
                <div className="p-4">
                  <h3 className="font-semibold text-gray-200 text-sm mb-2 line-clamp-2" title={file.original_name}>
                    {file.original_name}
                  </h3>
                  <div className="flex justify-between items-center text-xs text-gray-400">
                    <span>{file.formatted_size}</span>
                    <span className="hidden sm:inline">{new Date(file.uploaded_at).toLocaleDateString('sv-SE')}</span>
                  </div>
                  {file.folder_path && (
          ...
          <div className="space-y-3 sm:space-y-0">
            {/* Desktop table view */}
            <div className="hidden sm:block bg-white/10 backdrop-blur-sm rounded-lg shadow-sm overflow-hidden border border-white/20">
                  ...
                  {filteredFiles.map((file) => (
                    <tr
                      ...
                      className={`hover:bg-white/10 cursor-pointer transition-colors ${
                        selectedItems.includes(file.id) ? 'bg-yellow-500/20' : ''
                      }`}
                      onClick={() => toggleSelection(file.id)}
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm font-medium text-gray-200" title={file.original_name}>
                            {file.original_name}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          file.is_image ? 'bg-green-500/20 text-green-300' : 'bg-purple-500/20 text-purple-300'
                        }`}>
              ...
              {filteredFiles.map((file, index) => (
                <div
                  ...
                  className={`bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20 ${
                    selectedItems.includes(file.id) ? 'ring-2 ring-yellow-500 bg-yellow-500/10' : ''
                  }`}
                  onClick={() => toggleSelection(file.id)}
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center space-x-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="text-sm font-medium text-gray-200 truncate" title={file.original_name}>
                          {file.original_name}
                        </h3>
                    ...
                    <button
                      ...
                      className="ml-3 bg-yellow-600 hover:bg-yellow-700 text-white p-2 rounded-lg transition-colors flex-shrink-0"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                  ...
                  {file.is_image && (
                    <button
                      ...
                      className="mt-3 w-full bg-white/10 hover:bg-white/20 text-gray-200 py-2 rounded-lg transition-colors text-sm"
                    >
                      👁️ Förhandsgranska
                    </button>
                  )}
